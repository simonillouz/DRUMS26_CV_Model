function  [patientTable, S_history] = DriverBasic(PID,opt,teich,k,ColorMap,FS)
...
nexttile(1);
    ...
    fontsize(FS,"points")
    xLimits = [0, tend];
    plot(xLimits, [data.PpaS, data.PpaS], 'k:', 'linewidth', 2); hold on;
    plot(xLimits, [data.PpaD, data.PpaD], 'k:', 'linewidth', 2); hold on;
    ylim([0.95*min(min(States.Ppa(Is:Ie)), data.PpaD) 1.05*max(max(States.Ppa(Is:Ie)), data.PpaS)]);
    ylabel('Ppa (mmHg)');
nexttile(2)
    ...
    plot(xLimits, [data.Ppv, data.Ppv], 'k:', 'linewidth', 2);
    ylim([0.95*min(min(States.Ppv(Is:Ie)), data.Ppv) 1.05*max(max(States.Ppv(Is:Ie)), data.Ppv)]);    
    ylabel('Ppv (mmHg)');
nexttile(3)
...
nexttile(4)
    ...
    ylim([0.95*min(min(States.Psa(Is:Ie)), data.PsaD) 1.05*max(max(States.Psa(Is:Ie)), data.PsaS)]);
    ylabel('Psa (mmHg)');
nexttile(5)
...
nexttile(6)
    ...
    ylim([0.95*min(min(States.Prv(Is:Ie)), data.PrvD) 1.05*max(max(States.Prv(Is:Ie)), data.PrvS)]);
    ylabel('Prv (mmHg)');
% Volume vs Time
nexttile(7)
...
lgd = legend([hModel, hData, hCalculated], ...
    {'Model', 'Data', 'Calculated Value'}, ...
